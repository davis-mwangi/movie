...
public class JwtUsernameAndPasswordAuthenticationFilter extends UsernamePasswordAuthenticationFilter {
    ...
    protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain,
        ...
        UserModel principal = ((UserPrincipal) auth.getPrincipal()).getUser();
        ...
        String token = Jwts.builder()
                ...
                .signWith(SignatureAlgorithm.RS512, Util.getPrivateKey(Util.SB_FILE_PATH + "/private_key.der"))
                .compact();

        TokenResponse tokenResponse = new TokenResponse(token, exp);
        response.setHeader("Content-Type", MediaType.APPLICATION_JSON_VALUE);
        response.getOutputStream().println(Util.toJson(tokenResponse));
    }
